...
if PLATFORM == 'gcc':
    ...
    LFLAGS = DEVICE + ' -Wl,--gc-sections,-Map=rt-thread.map,-cref,-u,Reset_Handler -T board/linker_scripts/link.ld'
    ...
    DEVICE = ' --target=arm-none-eabihf -mfloat-abi=hard -march=armv7em -mfpu=fpv5-d16'
    DEVICE += ' -ffunction-sections -fdata-sections -fno-exceptions -fno-rtti'
    CFLAGS = DEVICE
    AFLAGS = ' -c' + DEVICE + ' -Wa,-mimplicit-it=thumb '
    LFLAGS = DEVICE + ' -Wl,--gc-sections,-Map=rt-thread.map,-u,Reset_Handler -lcrt0 -T board/linker_scripts/link.ld'
