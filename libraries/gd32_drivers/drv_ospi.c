 ...
 * 2026-08-31     CYFS         adapt OSPI0 for four-line QSPI flash
 */
...
{
    return data != RT_NULL &&
           ((rt_ubase_t)data & (GD32_OSPI_FIFO_WORD_SIZE - 1U)) == 0U &&
           length >= GD32_OSPI_MDMA_MIN_SIZE &&
           length <= GD32_OSPI_MDMA_MAX_SIZE &&
           (length & (GD32_OSPI_MDMA_BUFFER_SIZE - 1U)) == 0U;
}
...
static rt_err_t gd32_ospi_mdma_transmit(const rt_uint8_t *data, rt_size_t length)
{
    ...
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_FLUSH, (void *)data, (int)length);
#endif
...
#ifdef RT_USING_CACHE
    rt_hw_cpu_dcache_ops(RT_HW_CACHE_INVALIDATE, data, (int)length);
#endif
