...
# 2026-09-02     CYFS         add include paths for preprocessed assembly sources
# 2026-09-02     CYFS         preserve RT-Thread internal interface definitions
#
...
def HandleToolOption(tools, env, project, reset):
    ...
    CPPDEFINES = list(project['CPPDEFINES'])
    internal_defines = ['__RT_KERNEL_SOURCE__', '__RT_IPC_SOURCE__']
    for group in env.get('project', []):
        for cppdef in ListMap(group.get('LOCAL_CPPDEFINES', [])):
            if cppdef in internal_defines and cppdef not in CPPDEFINES:
                CPPDEFINES.append(cppdef)
    ...
    for tool in tools:
        ...
        if tool.get('id').find('compile') != 1:
            ...
            for option in options:
                ...
                if (('compiler.include.paths' in option_id) or
                    ('compiler.option.includepaths' in option_id) or
                    ('compiler.tasking.include' in option_id) or
                    ('assembler.include.paths' in option_id)):
