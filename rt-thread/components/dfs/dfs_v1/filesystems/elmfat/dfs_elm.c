 ...
 * 2026-09-01     CYFS         serialize logical drive allocation
 */
...
static struct rt_mutex volume_lock;
...
int dfs_elm_mount(struct dfs_filesystem *fs, unsigned long rwflag, const void *data)
{
    ...
    rt_mutex_take(&volume_lock, RT_WAITING_FOREVER);
...
__err:
    rt_kprintf("[E/elm] mount %s at %s failed: volume=%d, result=%d\n",
               fs->dev_id->parent.name, fs->path, index, result);
    f_mount(RT_NULL, (const TCHAR *)logic_nbr, 1);
    disk[index] = RT_NULL;
    rt_free(fat);
    status = elm_result_to_dfs(result);

__exit:
    rt_mutex_release(&volume_lock);
    return status;
}
...
int elm_init(void)
{
    if (rt_mutex_init(&volume_lock, "elmvol", RT_IPC_FLAG_PRIO) != RT_EOK)
    {
        return -RT_ERROR;
    }
