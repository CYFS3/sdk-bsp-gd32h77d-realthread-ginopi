 ...
 * Change Logs:
 * Date           Author       Notes
 * 2026-09-08     CYFS         validate command buffer lengths
 */
...
static void i2c(int argc,char *argv[])
{
    if(argc > 2)
    {
        ...
        if(!strcmp(argv[1], "read"))
        {
            ...
            {
                char *end;
                long read_len = strtol(argv[ARG_READ_MAX - 1], &end, 10);
                if(end == argv[ARG_READ_MAX - 1] || *end != '\0' ||
                   read_len < 1 || read_len > I2C_TOOLS_BUFFER_SIZE)
                {
                    rt_kprintf("[i2c] Read length must be 1-%d bytes\n", I2C_TOOLS_BUFFER_SIZE);
                    return;
                }
                len = (rt_uint8_t)read_len;
            }
        ...
        else if(!strcmp(argv[1], "write"))
        {
            ...
            if(argc - ARG_DATA_POS > I2C_TOOLS_BUFFER_SIZE)
            {
                rt_kprintf("[i2c] Write length must not exceed %d bytes\n", I2C_TOOLS_BUFFER_SIZE);
                return;
            }
